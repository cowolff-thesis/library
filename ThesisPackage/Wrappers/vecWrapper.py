    ...
    def step(self, actions: dict[AgentID, ActionType]) -> tuple[
        ...
        if np.any(list(terminations.values())) or np.any(list(truncations.values())):
            for agent in self.agents:
                infos[agent]["timestep"] = self.env.timestep
                infos[agent]["rewards"] = self.env.episode_rewards
...
class PettingZooVectorizationParallelWrapper():
    def __init__(self, make_fn, n_envs: int):
        self.num_envs = n_envs
        self.env = SubprocVecEnv([lambda: PettingZooAutoResetParallelWrapper(make_fn()) for _ in range(n_envs)])
        return
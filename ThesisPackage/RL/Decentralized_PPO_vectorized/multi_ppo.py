...
try:
    ...
    from ThesisPackage.RL.Decentralized_PPO_vectorized.agent import Agent
    from ThesisPackage.RL.Decentralized_PPO_vectorized.util import *
    from ThesisPackage.Wrappers.vecWrapper import PettingZooVectorizationParallelWrapper


class PPO_Multi_Agent:
    def __init__(self, env, num_steps=2048, gae=True, gamma=0.99, gae_lambda=0.95, update_epochs=4, normalize_obs=False, norm_adv=True, clip_coef=0.2, clip_vloss=True, ent_coef=0.01, vf_coef=0.5, max_grad_norm=0.5, target_kl=None, window_size=100, device="cpu"):
        self.env = env
        self.device = device
    ...
    def train(self, total_timesteps, learning_rate=2.5e-4, anneal_lr=True, num_minibatches = 256, tensorboard_folder="results/", exp_name = None, seed = 1, torch_deterministic = True, lr_auto_adjust = False, num_checkpoints = 0, checkpoint_path="models/checkpoints"):
        ...
        elif hasattr(self.env, "num_envs"):
            num_envs = self.env.num_envs
        else:
            num_envs = 1
        ...
        elif isinstance(self.env, ParallelEnv) or isinstance(self.env[0], wrappers.BaseWrapper) or hasattr(self.env, "num_envs"):
        ...
        for update in progressbar(range(1, num_updates + 1), redirect_stdout=True):
            if update % checkpoint_frequency == 0:
                self.save(f"{checkpoint_path}/checkpoint_{update}.pt")
            ...
            for step in range(0, self.num_steps):
                ...
                if isinstance(self.env, list):
                ...
                elif isinstance(self.env, PettingZooVectorizationParallelWrapper) or hasattr(self.env, "num_envs"):
                    ...
                    resets = [terminated or truncated for terminated, truncated in zip(compare_entries(terminated), compare_entries(truncated))]
                ...
                if hasattr(self.env, "num_envs"):
                    if any(resets):
                        true_indices = np.nonzero(resets)[0]
                        for index in true_indices:
                            if index % self.num_agents == 0:
                                lengths_record.append(self.env.timestep[index])
                                rewards_record.append(self.env.episode_rewards[index])
                                current_env.reset(index=index)
            ...
            if isinstance(self.env, list):
                ...
                b_obs = obs.reshape((-1,) + self.env.observation_space(current_env.agents[0]).shape)
                b_logprobs = logprobs.reshape(-1)
                if isinstance(action_space, gym.spaces.Tuple):
                    b_actions = actions.reshape((-1,) + num_actions)
                else:
                    b_actions = actions.reshape((-1,) + self.env.action_space(current_env.agents[0]).shape)
                b_advantages = advantages.reshape(-1)
                b_returns = returns.reshape(-1)
                b_values = values.reshape(-1)

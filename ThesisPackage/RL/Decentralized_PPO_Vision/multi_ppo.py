...
try:
    ...
    from ThesisPackage.RL.Decentralized_PPO_Vision.agent import Agent
    from ThesisPackage.RL.Decentralized_PPO_Vision.util import *
...
class PPO_Multi_Agent:
    ...
    def train(self, total_timesteps, learning_rate=2.5e-4, anneal_lr=True, num_minibatches = 256, tensorboard_folder="results/", exp_name = None, seed = 1, torch_deterministic = True):
        ...
        vision_obs = torch.zeros((self.num_steps, num_agents * num_envs) + current_env.observation_space(current_env.agents[0])["vision"].shape).to(self.device)
        utterance_obs = torch.zeros((self.num_steps, num_agents * num_envs) + current_env.observation_space(current_env.agents[0])["utterance"].shape).to(self.device)
        ...
        next_obs = flatten_list(next_obs)

        next_vision_obs = np.array([new_obs["vision"] for new_obs in next_obs])
        next_utterance_obs = np.array([new_obs["utterance"] for new_obs in next_obs])

        next_vision_obs = torch.Tensor(next_vision_obs).to(self.device)
        next_utterance_obs = torch.Tensor(next_utterance_obs).to(self.device)
        ...
        for update in progressbar(range(1, num_updates + 1), redirect_stdout=True):
            ...
            for step in range(0, self.num_steps):
                ...
                utterance_obs[step] = next_utterance_obs
                dones[step] = next_done

                # ALGO LOGIC: action logic
                with torch.no_grad():
                    action, logprob, _, value = self.agent.get_action_and_value(next_utterance_obs, next_vision_obs)
                ...
                next_obs = torch.Tensor(current_dones).to(self.device)
            ...
            with torch.no_grad():
                next_value = self.agent.get_value(next_utterance_obs, next_vision_obs).reshape(1, -1)
                if self.gae:
            ...
            # flatten the batch
            b_vision_obs = vision_obs.reshape((-1,) + current_env.observation_space(current_env.agents[0])["vision"].shape)
            b_utterance_obs = utterance_obs.reshape((-1,) + current_env.observation_space(current_env.agents[0])["utterance"].shape)
            ...
            for epoch in range(self.update_epochs):
                ...
                for start in range(0, batch_size, minibatch_size):
                    ...
                    _, newlogprob, entropy, newvalue = self.agent.get_action_and_value(b_utterance_obs[mb_inds], b_vision_obs[mb_inds], b_actions.long()[mb_inds].T)

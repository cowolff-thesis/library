...
class PPO_Multi_Agent:
    def __init__(self, env, test_env=None, num_minibatches = 256, num_steps=2048, gae=True, gamma=0.99, gae_lambda=0.95, update_epochs=4, normalize_obs=False, norm_adv=True, clip_coef=0.2, clip_vloss=True, ent_coef=0.01, vf_coef=0.5, max_grad_norm=0.5, target_kl=None, window_size=100, device="cpu"):
        ...
        elif isinstance(env, ParallelEnv) or isinstance(env, wrappers.BaseWrapper):
    ...
    def test(self, num_episode=50):
        obs, infos = self.test_env.reset()
        wins = []
        for j in range(num_episode):
            while True:
                ...
                with torch.no_grad():
                    ...
                    actions = reverse_flatten_list_with_agent_list(actions, self.test_env.agents)
                obs, reward, truncations, terminations, infos = self.test_env.step(actions[0])
    ...
    def train(self, total_timesteps, learning_rate=2.5e-4, anneal_lr=True, num_minibatches = 256, tensorboard_folder="results/", exp_name = None, seed = 1, torch_deterministic = True, lr_auto_adjust = False, learn_team=0):
        """
        ...
        if learn_team != 0:
            learning_agents = [key for key, agent in current_env.env.env.players.items() if agent.team == learn_team]
            num_agents = len([agent for key, agent in current_env.env.env.players.items() if agent.team == learn_team])
        else:
            learning_agents = [key for key, agent in current_env.players]
        ...
        learning_obs = {agent: cur_obs for agent, cur_obs in next_obs.items() if agent in learning_agents}
        learning_obs = concatenate_agent_observations(learning_obs)
        ...
        if self.normalize_obs:
            ...
            learning_obs = normalize_batch_observations(learning_obs, min_values, max_values)
        next_obs = torch.Tensor(next_obs).to(self.device)
        learning_obs = torch.Tensor(learning_obs).to(self.device)
        ...
        for update in progressbar(range(1, num_updates + 1), redirect_stdout=True):
            ...
            for step in range(0, self.num_steps):
                ...
                obs[step] = learning_obs
                dones[step] = next_done

                # ALGO LOGIC: action logic
                with torch.no_grad():
                    action, logprob, _, value = self.agent.get_action_and_value(next_obs)
                    learning_values = split_agent_torch(value, self.agents)
                    learning_values = {agent: cur_value for agent, cur_value in learning_values.items() if agent in learning_agents}
                    learning_values = concatenate_torch(learning_values)
                    values[step] = learning_values.flatten()

                learning_action = split_agent_torch(action, self.agents)
                learning_action = {agent: cur_action for agent, cur_action in learning_action.items() if agent in learning_agents}
                learning_action = concatenate_torch(learning_action)

                learning_logprob = split_agent_torch(logprob, self.agents)
                learning_logprob = {agent: cur_logprob for agent, cur_logprob in learning_logprob.items() if agent in learning_agents}
                learning_logprob = concatenate_torch(learning_logprob)

                actions[step] = learning_action
                logprobs[step] = learning_logprob
                ...
                learning_obs = {agent: cur_obs for agent, cur_obs in new_obs.items() if agent in learning_agents}
                learning_obs = concatenate_agent_observations(learning_obs)
                learning_obs = torch.Tensor(learning_obs).to(self.device)
                
                learning_rewards = {agent: cur_reward for agent, cur_reward in reward.items() if agent in learning_agents}
                learning_rewards = concatenate_agent_observations(learning_rewards)
                ...
                learning_dones = split_agent_actions(current_dones, self.agents)
                learning_dones = {agent: cur_done for agent, cur_done in learning_dones.items() if agent in learning_agents}
                learning_dones = concatenate_agent_observations(learning_dones)
                ...
                rewards[step] = torch.tensor(learning_rewards.flatten()).to(self.device).view(-1)
                next_obs, next_done = torch.Tensor(next_obs).to(self.device), torch.Tensor(learning_dones).to(self.device)
            ...
            with torch.no_grad():
                next_value = self.agent.get_value(learning_obs).reshape(1, -1)
                if self.gae:
            ...
            if exp_name is not None:
                ...
                if self.test_env is not None:
                    writer.add_scalar("test/team_one_wins", test_one / len(test_wins), global_step)
                    writer.add_scalar("test/team_two_wins", test_two / len(test_wins), global_step)

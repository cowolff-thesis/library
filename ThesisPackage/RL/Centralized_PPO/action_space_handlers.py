...
def handle_multidiscrete(actor, critic, action_space, inputs, state, action=None):
    logits = actor(inputs)
    ...
    return action.T, logprob.sum(0), entropy.sum(0), critic(state)

def handle_discrete(actor, critic, inputs, state, action=None):
    logits = actor(inputs)
    ...
    return action, logprob, entropy, critic(state)


def handle_box(actor, critic, actor_logstd, inputs, state, action=None):
    action_mean = actor(inputs)
    action_logstd = actor_logstd.expand_as(action_mean)
    action_std = torch.exp(action_logstd)
    probs = Normal(action_mean, action_std)
    if action is None:
        action = probs.sample()
    return action, probs.log_prob(action).sum(1), probs.entropy().sum(1), critic(state)
...
def handle_tuple(actor, critic, action_space, actor_logstd, inputs, state, action=None):
    logits = actor(inputs)
    actions = []
    logprobs = []
    entropies = []
    ...
    return actions, logprobs, entropies, critic(state)

...
try:
    ...
    from ThesisPackage.RL.Centralized_PPO_LSTM.agent import Agent
    from ThesisPackage.RL.Centralized_PPO_LSTM.util import *
...
class PPO_Multi_Agent_Centralized:
    ...
    def train(self, total_timesteps, learning_rate=2.5e-4, anneal_lr=True, num_minibatches = 256, tensorboard_folder="results/", exp_name = None, seed = 1, torch_deterministic = True, num_checkpoints = 0, checkpoint_path="models/checkpoints"):
        ...
        next_lstm_state = (
            torch.zeros(self.agent.lstm.num_layers, num_agents * num_envs, self.agent.lstm.hidden_size).to(self.device),
            torch.zeros(self.agent.lstm.num_layers, num_agents * num_envs, self.agent.lstm.hidden_size).to(self.device),
        )
        ...
        for update in progressbar(range(1, num_updates + 1), redirect_stdout=True):
            ...
            assert (num_envs * num_agents) % num_minibatches == 0
            total_env_agents = num_envs * num_agents
            # Total batch size: T * (envs * agents)
            batch_size = self.num_steps * total_env_agents
            # Number of (env, agent) groups per minibatch
            envs_per_batch = total_env_agents // num_minibatches

            # We reshape the indices to [num_steps, num_envs * num_agents]
            flat_inds = np.arange(batch_size).reshape(self.num_steps, total_env_agents)
            # Create an array of environment-agent indices (0..(num_envs*num_agents - 1))
            env_agent_inds = np.arange(total_env_agents)

            clipfracs = []

            for epoch in range(self.update_epochs):
                # Shuffle the environment-agent pairs
                np.random.shuffle(env_agent_inds)

                for start in range(0, total_env_agents, envs_per_batch):
                    end = start + envs_per_batch
                    mb_env_agent_inds = env_agent_inds[start:end]

                    # Flatten the chosen environment-agent pairs across time
                    mb_inds = flat_inds[:, mb_env_agent_inds].ravel()

                    # Select the appropriate LSTM hidden states for these environment-agent pairs
                    # initial_lstm_state is (h, c) with shape [num_layers, total_env_agents, hidden_size]
                    mb_lstm_state = (
                        initial_lstm_state[0][:, mb_env_agent_inds, :].contiguous(),
                        initial_lstm_state[1][:, mb_env_agent_inds, :].contiguous(),
                    )

                    # Forward pass: compute new logprobs, values, etc.
                    # Make sure your agent uses these LSTM states and done flags appropriately
                    _, newlogprob, entropy, newvalue, _ = self.agent.get_action_and_value(
                        b_obs[mb_inds],
                        mb_lstm_state,
                        b_dones[mb_inds],
                        b_actions[mb_inds].T,   # might be .long() if discrete
                    )
                ...
                # Optional early stopping based on target KL

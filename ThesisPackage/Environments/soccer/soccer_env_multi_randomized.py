...
try:
    ...
    from util import player_position_relative, get_direction_relative, generate_random_coordinates
except ImportError:
    from ThesisPackage.Environments.soccer.game import Player, Ball
    from ThesisPackage.Environments.soccer.util import player_position_relative, get_direction_relative, generate_random_coordinates

class SoccerGame(ParallelEnv):
    metadata = {'render.modes': ['human']}

    def __init__(self, width=7, height=5, sequence_length=1, vocab_size=1, max_timesteps=1024, noise_team=[]):
        super().__init__()
        self.width = width
        self.height = height
        # randomly initialize the players positions
        player_1_pos, player_3_pos = generate_random_coordinates((1, self.width // 2 - 1), (1, self.height - 1), 1)
        player_2_pos, player_4_pos = generate_random_coordinates((self.width // 2 + 1, self.width - 2), (1, self.height - 1), 1)
        self.players = {
            "player_1": Player(player_1_pos, self.width - 1, "player_1", 1),
            "player_2": Player(player_2_pos, 0, "player_2", -1),
            "player_3": Player(player_3_pos, self.width - 1, "player_3", 1),
            "player_4": Player(player_4_pos, 0, "player_4", -1)
        }
    ...
    def reset(self, seed=1, options=None):
        ...
        self.players['player_1'].position = player_1_pos
        self.players['player_2'].position = player_2_pos
        self.players['player_3'].position = player_3_pos
        self.players['player_4'].position = player_4_pos
        self.timestep = 0
        self.episode_rewards = 0
        self.utterances = {agent: [0 for _ in range(self.sequence_length)] for agent in self.agents}
    ...
    def observe(self, agent):
        ...
        for other_agent in other_agents:
            if self.players[other_agent].team == player.team:
                if player.team in self.noise_team:

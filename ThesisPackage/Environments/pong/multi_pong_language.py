...
class PongEnv(ParallelEnv):
    ...
    def __init__(self, width=20, height=10, paddle_height=3, sequence_length=1, vocab_size=2, max_episode_steps=1024):
        ...
        self.balls = {"ball_1": {"position": [self.width // 2 - 1, self.height // 2], "direction":[random.uniform(-0.9, 0.9), random.uniform(-0.9, 0.9)]},
                        "ball_2": {"position": [self.width // 2 + 1, self.height // 2], "direction":[random.uniform(-0.9, 0.9), random.uniform(-0.9, 0.9)]}}

...
class PongEnv(ParallelEnv):
    ...
    def __move_ball(self, ball_pos, ball_direction):
        ...
        indices = {paddle: np.where(rewards[paddle] == 1)[0] for paddle in self.paddles.keys()}
        result = np.unique(np.concatenate((indices["paddle_1"], indices["paddle_2"])))

        ball_direction[:,0][result] = ball_direction[:,0][result] * -1
        ball_pos[result] += ball_direction[result]
    ...
    def one_hot_encode(self, arr):
        ...
        # Use numpy.eye to create the identity matrix and use indexing to get the one-hot encoded vectors
        one_hot_encoded = np.eye(self.vocab_size)[arr]

        flattened = one_hot_encoded.reshape(one_hot_encoded.shape[0], -1)
    ...
    def __get_observation(self, paddle):
        ...
        utterance_obs = self.one_hot_encode(self.utterances[other_paddle])
        obs = np.concatenate((obs, utterance_obs), axis=1)
        return obs
    ...
    def step(self, actions):
        ...
        for paddle in actions.keys():
            ...
            self.paddles[paddle] = np.clip(self.paddles[paddle], 0, self.height)

        if self.sequence_length > 0:
            for paddle in self.paddles.keys():
                self.utterances[paddle] = actions[paddle][:,1:].astype(np.int8)
    ...
    def reset(self, index=None, seed=1, options=None):
        if index == None:
            ...
            self.utterances = {agent: np.array([[0 for _ in range(self.sequence_length)] for env in range(self.num_envs)], dtype=np.int8) for agent in self.agents}
            ...
            for ball in self.balls.keys():
                self.balls[ball]["position"][index] = np.array([self.width // 2 - 1, self.height // 2])
                self.balls[ball]["direction"][index] = np.array([random.uniform(0.5, 1), random.uniform(0.5, 1)])

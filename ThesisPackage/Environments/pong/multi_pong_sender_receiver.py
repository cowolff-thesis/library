...
class PongEnvSenderReceiver(ParallelEnv):
    """
    ...
    def __init__(self, width=20, height=10, paddle_height=3, sequence_length=1, vocab_size=2, max_episode_steps=1024, self_play=True, receiver=None, mute_method="random"):
        """
        ...
        """
        super(PongEnvSenderReceiver, self).__init__()
        ...
        if mute_method not in ["random", "zero"]:
    ...
    def __create_observation_space(self):
        ...
        if self.self_play:
            ...
            low_receiver = low + [0 for _ in range(self.sequence_length)]
            high_receiver = high + [self.vocab_size for _ in range(self.sequence_length)]
    ...
    def __get_observation(self, paddle):
        ...
        if self.self_play and paddle != self.receiver and self.receiver is not None:
            if self.mute_method == "zero":
                obs = np.append(obs, [0 for _ in range(self.sequence_length)])

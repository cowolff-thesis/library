...
try:
    from game import Player, Target
    from util import position_relative, get_direction_relative, generate_random_coordinates
except ImportError:
    from ThesisPackage.Environments.collectors.game import Player, Target
    from ThesisPackage.Environments.collectors.util import position_relative, get_direction_relative, generate_random_coordinates
...
class Collectors(ParallelEnv):
    ...
    def __init__(self, width=7, height=5, sequence_length=1, vocab_size=1, num_targets=3, timestep_countdown=20, spawn_frequency = 10, frequency_range=7, max_timesteps=1024):
        ...
        player_pos = generate_random_coordinates((1, self.width - 1), (1, self.height - 1))
        self.players = {
            "player_1": Player(copy.deepcopy(player_pos), "player_1"),
            "player_2": Player(copy.deepcopy(player_pos), "player_2"),
        }
        ...
        self._discrete_space = spaces.MultiDiscrete([self.vocab_size for _ in range(self.sequence_length)])
        if self.sequence_length == 0:
            self._action_space = {agent: self._move_space for agent in self.agents}
        else:
    ...
    def __get_observation_space_critic(self):
        ...
        for agent in self.agents:
            ...
            observation_high = np.concatenate((observation_high, np.array([self.vocab_size for _ in range(self.sequence_length)])))
            observation_low = np.concatenate((observation_low, np.array([0 for _ in range(self.sequence_length)])))
    ...
    def __get_observation_space_player(self):
        ...
        for sequence in range(self.sequence_length):
            observation_high = np.concatenate((observation_high, np.array([self.vocab_size])))
            observation_low = np.concatenate((observation_low, np.array([0])))
    ...
    def reset(self, seed=1, options=None):
        ...
        self.targets = []
        self.targets.append(Target(generate_random_coordinates((1, self.width - 1), (1, self.height - 1)), self.timestep_countdown))
    ...
    def step(self, actions):
        ...
        for target in range(length):
            ...
            for agent in self.agents:
                ...
                if np.linalg.norm(self.players[agent].get_position() - self.targets[target].get_position()) < 0.5:
        ...
        for agent, action in actions.items():
            self.utterances[agent] = action[2:]
    ...
    def observe(self, agent):
        ...
        other_utterance = np.array(self.utterances[other_agents], dtype=np.float32)
